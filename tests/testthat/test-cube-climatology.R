...
test_that("provenance records the complete scientific contract", {
  ...
  expect_identical(record$operation, "cube_climatology")
  expect_identical(record$parameters$requested$period, range(x$time))
  expect_identical(record$parameters$resolved$period_effective, range(x$time))
  expect_false(record$parameters$resolved$period_clipped)
  expect_identical(record$parameters$requested$by, "month")
  expect_true(is.na(record$parameters$requested$leap))
  expect_identical(record$parameters$requested$min_n, 1L)
  expect_identical(record$parameters$resolved$center, "arithmetic_mean")
  expect_identical(record$parameters$resolved$sd_method, "sample_standard_deviation")
  expect_identical(record$parameters$resolved$inner_weighting, "equal_observation")
  expect_identical(record$parameters$resolved$outer_weighting, "equal_period_replicate")
  ...
  expect_identical(result$provenance$time$current$kind, "recurring_climatology")
