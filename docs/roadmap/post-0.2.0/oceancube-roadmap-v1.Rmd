...
and approval state; it does not authorize implementation by itself. The stable
release baseline remains version 0.2.0 with 38 exports; the active
0.2.0.9000 development runtime has 39 after A5b.
...
**Status: UNSATISFIED after the 0.3.0-A hardening exit.** Completion of
`0.3.0-A` permits staged `0.3.0-B` CF/interoperability work; it does not itself
authorize vertical science.
...
deferred-NetCDF entry. `read_nc()` remains materializing. A5a resolves
`DEC-018` by approving one experimental `cube_open()` entry for the existing
local read-only deferred NetCDF backend; A5b implements and certifies that
exact entry without changing the eager reader.
...
at the A4R close, A5 was next.

**0.3.0-A5a status (2026-08-26): public deferred NetCDF API architecture
decision completed.** Current runtime, tests, serialization, file identity,
operation behavior, synthetic/OISST parity, rough performance/allocation, and
external API patterns support one experimental `cube_open()` entry for one
local read-only NetCDF source. `read_nc()` remains eager, `vars = NULL` is a
metadata-only variable-discovery contract, the deferred result retains class
`c("ocean_cube", "list")`, and `cube_collect()` is the explicit transition to
memory. `DEC-018` is approved and `A1-004` is partially closed pending A5b
implementation/certification. No runtime, tests, dependency, NAMESPACE,
DESCRIPTION, or export changes occur in A5a. 0.3.0-A remains incomplete and
Gate B remains unsatisfied; A5b is next.

**0.3.0-A5b status (2026-08-26): public deferred NetCDF API implementation
certified.** The exact experimental `cube_open()` signature is the sole new
export. It opens one local read-only NetCDF file as the existing
`ocean_cube/list` class with versioned storage and no `x$data`, discovers
`vars = NULL` from metadata in source order, retains bounded operations and
serializable size/mtime identity, and records canonical `read_nc` followed by
explicit `cube_collect` when materialized. Public synthetic and governed OISST
tests pass; the known eager missing-value sentinel remains classified parser
debt. `read_nc()` and scientific results are unchanged, `DEC-018` is
implemented/certified, and `A1-004` is closed. At the A5b close, 0.3.0-A and
Gate B remained incomplete because `A1-009` peak-memory/stress evidence was
open; A6 was next.

**0.3.0-A6 status (2026-08-27): peak memory, stress, and hardening performance
certified.** A live x64 R process monitor calibrated at 1.02875 times a touched
128 MiB allocation. Fresh isolated workers measured all required memory,
eager, deferred, temporal, and bounded operations across TINY/SMALL/MEDIUM;
the governed OISST smoke and 75-operation plus 50-descriptor/error stress pass.
There is no RED memory classification, connection delta is zero, and the file
rename probe passes. Sparse NetCDF slice/extract envelope amplification remains
the non-blocking AMBER finding `A6-001`. `A1-009` is closed and A6 is complete
without runtime or dependency changes. 0.3.0-A and Gate B remain incomplete
pending the separate A-EXIT certification.

**0.3.0-A-EXIT status (2026-08-27): hardening COMPLETE/CERTIFIED.** The global
exit review reconciles A1 through A6, all 16 hardening findings, zero unresolved
S0/S1 blockers, the 39-export API, eager/deferred boundaries, Provenance V1,
coast reproducibility, governed real data, backend parity, invariances,
scientific/time/privacy/serialization regression, and A6 memory/stress
integrity. Remaining findings are explicitly accepted nonblocking debt,
0.3.0-B inputs, or optional external integration. `0.3.0-A complete = TRUE`
and `ready to begin 0.3.0-B = TRUE`; Gate B remains UNSATISFIED because its
canonical meaning is permission to begin vertical science and the CF/vertical
coordinate and primitive prerequisites are not yet approved.

**0.3.0-B1 status (2026-08-27): CF metadata foundation and engine decision
COMPLETE.** CF 1.13 is the normative design target. B1 audits both readers,
defines preserve/interpret/validate/use stages, a versioned plain-R model at
`x$metadata$cf`, raw plus interpreted links, conflict-aware axis evidence, and
source/current propagation. Native real/synthetic/group prototypes and an
isolated ncdfCF 0.8.2 comparison approve `DEC-015` as HYBRID: oceancube owns
the canonical model and lightweight scanner/interpreter; ncdfCF remains an
optional oracle/reference/adapter. Runtime, dependencies, API, and scientific
results are unchanged. `DEC-023`, `A3B-001`, `A3B-002`, and `A3B-003` remain
open; Gate B remains UNSATISFIED.

**0.3.0-B2 status (2026-08-28): CF metadata preservation engine implementation
and local package certification COMPLETE, pending remote CI.** The production
foundation implements `oceancube_metadata 1.0.0` and
`oceancube_cf_metadata 1.0.0` at `x$metadata$cf`, one path-qualified scanner,
raw plus simple interpreted links, strict plain-R validation, one
conflict-aware resolver shared by eager and deferred ingestion, exact
`cube_collect()` preservation, and an explicit pending-derivation state for
scientifically changing transformations. OISST `zlev` resolves from
`positive=down` without overrides in both readers, closing `A3B-001` with
exact numerical parity. ETOPO remains unrepresentable as a temporal cube and
WOA months-since remains rejected; `A3B-002`, `A3B-003`, and `DEC-023` remain
open. No dependency, export, calendar, static-field, or vertical-science scope
was added; Gate B remains UNSATISFIED.

**0.3.0-B3 status (2026-08-28): CF supported-subset interpretation and
validation implementation and package certification COMPLETE.** B3 activates
the versioned `oceancube_supported_subset` validator
against CF 1.13 requirements/recommendations without claiming full
conformance. It adds deterministic source/current diagnostics, bounded
coordinate/link/range/flag interpretation, explicit value/unit/table/grammar
deferrals, and no-payload-read proof while preserving the B2 source tree,
public API, dependencies, scientific values, and time behavior. OISST, ETOPO,
and WOA source scans pass the documented subset; ETOPO still has no fabricated
time and WOA still rejects `months since`. `A3B-001` remains closed;
`A3B-002`, `A3B-003`, and `DEC-023` remain open; Gate B remains UNSATISFIED.

**0.3.0-B4 status (2026-08-28): CF time and calendar representation decision
COMPLETE.** `DEC-023` is approved as a HYBRID CALENDAR-AWARE TIME MODEL.
Current exactly representable Date/POSIXct behavior remains unchanged; future
non-base-representable CF calendars use an oceancube-owned versioned plain-R
state, with CFtime only as a reconstructable transient arithmetic adapter and
oracle. The calendar taxonomy, comparison, selection, grouping, climatology,
trend metric, serialization, table/plot, provenance, UTC/TAI, `none`, custom
calendar and WOA ambiguity policies are fixed in
`inst/architecture/oceancube-cf-time-calendar-v1.md`, with executable and
matrix evidence under `dev/hardening/cf-time/`. B4 changes no runtime,
dependency, export, decoder or scientific result. `A3B-003` remains open with
architecture resolved and runtime/provider adjudication pending; `A3B-002`
also remains open. `0.3.0-B` is IN PROGRESS and Gate B remains UNSATISFIED.

**0.3.0-B5 status (2026-08-28): CF time and calendar engine implementation and
local package certification COMPLETE.** The internal plain-R
`oceancube_cf_time 1.0.0` engine implements fixed elapsed units for mixed
`standard`/`gregorian`, `proleptic_gregorian`, `julian`, `365_day`/`noleap`,
`366_day`/`all_leap`, and `360_day`, while every previously representable
Date/POSIXct case remains unchanged. Eager/deferred reading, validation,
selection, crop, collect, tables, serialization and non-temporal carrying are
calendar-safe; unsupported Tier-2 temporal science errors explicitly. CFtime
remains optional development evidence and is not a dependency. DEC-023 is
APPROVED — CORE RUNTIME IMPLEMENTED/CERTIFIED. WOA months-since and A3B-003,
ETOPO/static time and A3B-002, and Gate B remain open. The next bounded subphase
is B6 climatological time and WOA23 adjudication; it is not executed by B5.
...
The B1-approved model covers `standard_name`, `long_name`, `units`, `coordinates`, `axis`,
`positive`, `bounds`, `cell_methods`, `cell_measures`, `grid_mapping`,
`ancillary_variables`, `flag_values`, `flag_masks`, calendars, and formula
terms. `DEC-015` selects a HYBRID engine: the canonical plain-R model and
lightweight scanner/interpreter are owned by oceancube, while `ncdfCF` is an
optional adapter/reference implementation/conformance oracle and is never
embedded as canonical state. B2 implements the preservation foundation and
simple links. B3 implements the documented metadata-only structural subset,
with explicit deferrals instead of unsupported conformance claims. Full
grammar, full conformance, calendars, grid transformations, and formula
...
and are not isolated date helpers. B4 approves `DEC-023` as a hybrid model and
B5 implements its bounded core: exact Date/POSIXct cases remain while non-base
calendars use oceancube-owned plain state. CFtime is an optional development
oracle, not production state or a dependency. Climatological months-since and
WOA23 provider adjudication remain B6.
...
| Deferred NetCDF backend public entry | CLOSED in A5b | Retain the exact experimental `cube_open()` regression contract |
...
`0.3.0-A` is COMPLETE/CERTIFIED. B1-B7 establish the bounded CF foundation;
DEC-029 satisfies Gate B only for dimensional metric ocean depth with certified
CF semantics and explicit bounds. C1-C10 are technically COMPLETE/CERTIFIED and
the post-C-EXIT development API contained 48 exports. C-GOVR reconciles the C8-C10 decisions
DEC-037 through DEC-039 into the canonical register and documents why
`dev/hardening/**` remains repository-only evidence under the intentional
source-build exclusion. C-EXIT certifies the complete bounded C1-C10 chain
locally and remotely through DEC-040 without runtime or API change. Thus
`0.3.0-C` is COMPLETE/CERTIFIED. D1A adds DEC-041, a persistent reference
registry, global visualization inventory, renderer evaluation,
renderer-neutral architecture, and gallery governance without changing
runtime, tests, dependencies, exports, or version. D1B adds DEC-042 and the
internal renderer-neutral prepared-data contract while preserving the five
existing visual APIs and outputs; D1A and D1B are certified locally and
remotely. D2A adds only `viz.hovmoller()`, bringing the development API to 49
exports, and technically validates time-longitude, time-latitude, and time-
depth views plus the internal scale foundation. DEC-043 records maintainer
`qselmer` approval of the exact three
governed Hovmöller gallery artifacts on 2026-09-06 and marks D2A
COMPLETE/CERTIFIED. Stored-centre tile extent remains explicitly display-only;
grey stored `NA` tiles are distinct from blank regions without stored centres,
and explicit-bounds runtime support remains deferred. D1A and D1B remain
COMPLETE/CERTIFIED. D2B additively implements three safe map styles, explicit
scientific scale classes, display-only longitude wrapping, and governed
composition without a new export or dependency. Maintainer `qselmer` approved
the exact seven governed D2B artifacts on 2026-09-07. DEC-045 marks D2B and the
complete D2A+D2B core D2 scope COMPLETE/CERTIFIED LOCALLY. Phase `0.3.0-D`
remains IN PROGRESS and D3 is NOT STARTED.
The subsequent D-VIZDATA infrastructure phase adopts the NOAA/NCEP GODAS 2024
