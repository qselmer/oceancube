...
local read-only deferred NetCDF backend; implementation remains separately
gated as A5b.
...
complete A4 sequence through A4-EXIT, A4R, and A5a have evidence; A4 is closed,
coast distance is reproducible, and the public deferred-NetCDF architecture is
approved. The single recommended next subphase is A5b, implementation and
certification of the exact A5a `cube_open()` contract. `DEC-018` is approved
and `A1-004` is partially closed; 0.3.0-A remains incomplete and Gate B remains
unsatisfied. This placement does not authorize CF, vertical, provider,
multifile, remote, Zarr, 3-D, A6, or broader A5 runtime implementation.
