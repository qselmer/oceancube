...
and are not isolated date helpers. B4 approves `DEC-023` as a hybrid model:
current exact Date/POSIXct cases remain, while non-base calendars use future
oceancube-owned plain state with a transient reconstructable CFtime adapter.
...
in the staged 0.3.0-B CF/interoperability block. B1, B2, and B3 are COMPLETE
locally and DEC-015 is APPROVED — HYBRID ACTIVE SUPPORTED-SUBSET ENGINE. Gate
B remains UNSATISFIED and vertical science is not authorized. The single
recommended next subphase is `0.3.0-B4 — CF TIME AND CALENDAR REPRESENTATION
DECISION`: WOA/climatology and all later temporal operations require one safe
calendar model, while static optional-time support can follow without forcing
an unresolved temporal representation into the cube contract. This placement
does not itself implement
vertical, provider, multifile, remote, Zarr, or 3-D behavior.
