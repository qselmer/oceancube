...
## Purpose
Mask cells by polygon using cell centres.
...
## Real signature
```r
cube_mask(x, polygons, crs = NULL, keep = c("inside", "outside"),      boundary = c("include", "exclude"))
```
...
- `keep`: see the installed Rd manual for validation and defaults.
- `boundary`: see the installed Rd manual for validation and defaults.

## Input and output
Input: **ocean_cube + polygon**. Output: **ocean_cube**.
...
```r
library(oceancube)
stopifnot(is.function(oceancube::cube_mask))
names(formals(oceancube::cube_mask))
```

## Example in a workflow
Typical position: `cube_crop` → `cube_mask()` → `analysis`. See the workflow scripts for a checked end-to-end example.
