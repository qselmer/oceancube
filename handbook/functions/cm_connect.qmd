...
## Purpose
Connect to the configured Copernicus client.
...
## Real signature
```r
cm_connect(env = "oceancube-copernicus", required = TRUE, module = "copernicusmarine",      verbose = TRUE)
```

## Arguments
- `env`: see the installed Rd manual for validation and defaults.
- `required`: see the installed Rd manual for validation and defaults.
...
## Input and output
Input: **configuration**. Output: **connection**.
...
```r
library(oceancube)
stopifnot(is.function(oceancube::cm_connect))
names(formals(oceancube::cm_connect))
```

## Example in a workflow
Typical position: `cm_setup` → `cm_connect()` → `download_nc`. See the workflow scripts for a checked end-to-end example.
...
## Related functions
Previous: `cm_setup`. Next: `download_nc`.
