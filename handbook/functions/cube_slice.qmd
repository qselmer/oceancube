...
## Purpose
Select discrete coordinates or indices.
...
## Real signature
```r
cube_slice(x, longitude = NULL, latitude = NULL, depth = NULL,      time = NULL, variable = NULL, by = c("value", "index"), match = c("exact",          "nearest"), tolerance = NULL)
```
...
```r
library(oceancube)
stopifnot(is.function(oceancube::cube_slice))
names(formals(oceancube::cube_slice))
```

## Example in a workflow
Typical position: `ocean_cube` → `cube_slice()` → `cube_extract`. See the workflow scripts for a checked end-to-end example.
