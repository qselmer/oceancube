...
## Purpose
Materialise a lazy cube in memory.
...
```r
library(oceancube)
stopifnot(is.function(oceancube::cube_collect))
names(formals(oceancube::cube_collect))
```

## Example in a workflow
Typical position: `read_nc` → `cube_collect()` → `cube_slice/cube_crop`. See the workflow scripts for a checked end-to-end example.
...
## Related functions
Previous: `read_nc`. Next: `cube_slice/cube_crop`.
