...
## Real signature
```r
cube_layer_thickness(x, depth_bounds = NULL, unit = c("native", "m", "km"))
```
...
## Input and output
Input: **ocean_cube**. Output: **vector/array**.
...
```r
library(oceancube)
stopifnot(is.function(oceancube::cube_layer_thickness))
names(formals(oceancube::cube_layer_thickness))
```

## Example in a workflow
Typical position: `ocean_cube` → `cube_layer_thickness()` → `cube_cell_volume`. See the workflow scripts for a checked end-to-end example.
...
## Related functions
Previous: `ocean_cube`. Next: `cube_cell_volume`.
